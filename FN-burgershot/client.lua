...
local time = config.minigametime
local circles = config.minigamecircles
...
AddEventHandler("FN-burgershot:tabuleiro1", function()
    ...
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "Tabuleiro1", {
        maxweight = config.Board1maxweight,
        slots = config.Board1slots,
    })
end)
...
AddEventHandler("FN-burgershot:tabuleiro2", function()
    ...
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "Tabuleiro2", {
        maxweight = config.Board2maxweight,
        slots = config.Board2slots,
    })
end)
...
AddEventHandler("FN-burgershot:tabuleiro3", function()
    ...
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "Tabuleiro3", {
        maxweight = config.Board3maxweight,
        slots = config.Board3slots,
    })
end)
...
AddEventHandler("FN-burgershot:congelador", function()
    if onDuty == true then
        ...
        maxweight = config.freezermaxweight,
        slots = config.freezerslots,
        })
    else
...
AddEventHandler("FN-burgershot:Frigorifico1", function()
    if onDuty == true then
        ...
        maxweight = config.fridge1maxweight,
        slots = config.fridge1slots,
        })
    else
        if config.notify == "qbcore" then
            QBCore.Functions.Notify(Lang.needduty, 'error', 7500)
...
AddEventHandler("FN-burgershot:Frigorifico2", function()
    if onDuty == true then
        ...
        maxweight = config.fridge2maxweight,
        slots = config.fridge2slots,
        })
    else
        if config.notify == "qbcore" then
            QBCore.Functions.Notify(Lang.needduty, 'error', 7500)
...
AddEventHandler("FN-burgershot:Frigorifico3", function()
    if onDuty == true then
        ...
        maxweight = config.fridge3maxweight,
        slots = config.fridge3slots,
        })
    else
        if config.notify == "qbcore" then
            QBCore.Functions.Notify(Lang.needduty, 'error', 7500)
    		...
    		if HasItems then
				...
				QBCore.Functions.Progressbar("fazer_moneyshot", Lang.Makemoneyshot, 4000, false, true, {
                    ...
                    if success then
                        ...
                        if config.notify == "qbcore" then
                            QBCore.Functions.Notify(Lang.tryagain, 'error', 7500)
                        elseif config.notify == "mythic" then
                            exports['mythic_notify']:DoHudText('error', Lang.tryagain)
                        end
                    end
				end, function()
					TriggerEvent('inventory:client:busy:status', false)
...
AddEventHandler("FN-burgershot:tirarfaca", function()
    if onDuty then
        QBCore.Functions.TriggerCallback('FN-burgershot:server:temfaca', function(HasItems)
            if HasItems then
                if config.notify == "qbcore" then
                    QBCore.Functions.Notify(Lang.haveaknife, 'error', 7500)
                elseif config.notify == "mythic" then
                    exports['mythic_notify']:DoHudText('error', Lang.haveaknife)
                end
            else
            ...
            QBCore.Functions.TriggerCallback('FN-burgershot:server:itemsdoor1', function(HasItems)
                if HasItems then
                    ...
                    QBCore.Functions.Progressbar('entregar_door1', Lang.Entregarcomida, math.random(1500, 3000), false, true, {
                        ...
                        TriggerServerEvent('FN-burgershot:server:payment')
                        done = true
                    end, function ()
                    end)
                else
...
Citizen.CreateThread(function()
    ...
    exports['qb-target']:AddCircleZone("faca", vector3(-1200.03, -895.64, 14.42), 0.3, {
        name = "faca",
        useZ=true,
    }, {
        options = {
